...
export const Header = () => {
  ...
  const renderDropdown = (categoryName: string, label: string, iconName: string, tools: typeof AI_TOOLS) => {
    ...
    return (
      <div className="relative group dropdown-container pb-2" onMouseEnter={() => toggle(dropdownKey)} onMouseLeave={() => toggle(dropdownKey)}>
        <button
          className="flex min-w-0 items-center space-x-1 text-sm font-medium text-gray-600 hover:text-[var(--color-primary-foreground)] transition-colors py-2"
                  ...
                  .map(tool => {
                    ...
                    return (
                      <Link key={tool.path} href={tool.path} onClick={handleMenuClose} className="flex items-center space-x-2 p-2 hover:bg-gray-50 rounded-lg transition-colors">
                        ...
                        <span className="text-sm text-[var(--color-primary-foreground)] hover:text-[var(--color-primary-foreground)]">{t(tool.translationKey)}</span>
                      </Link>
                    );
                  })}
  ...
  return (
    <header className="bg-[var(--color-sidebar-primary)] text-[var(--color-primary-foreground)] border-b border-gray-100 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 gap-2">
          ...
          <Link href="/" className="flex items-center cursor-pointer min-w-0">
            ...
            <span className="ml-2 text-xl font-bold truncate max-w-[10rem] sm:max-w-none">ImageConverter</span>
          </Link>
              ...
              {categories.map(cat => {
                ...
                return (
                  <div key={cat.name} className="dropdown-container">
                    <button onClick={() => toggle(`mobile-${cat.name}`)} className="flex items-center justify-between w-full text-left p-2 hover:bg-[var(--color-primary-foreground)] rounded-lg">
                    ...
                    {isOpen(`mobile-${cat.name}`) && (
                      <div className="ml-8 mt-2 space-y-2 max-h-[50vh] overflow-y-auto">
                        {cat.tools.map(tool => {
                          const ToolIcon = getIcon(tool.icon);
                          return (
                            <Link key={tool.path} href={tool.path} onClick={handleMenuClose} className="flex items-center space-x-2 p-2 hover:bg-[var(--color-primary-foreground)] rounded-lg">
                              <ToolIcon size={16} className="text-gray-500" />
                              <span className="text-sm">{t(tool.translationKey)}</span>
                            </Link>
                          );
                        })}
                      </div>
